...
priv_stmt='CREATE USER "db_replica_slave_user"@"%" IDENTIFIED BY "db_replica_slave_pwd"; GRANT REPLICATION SLAVE ON *.* TO "db_replica_slave_user"@"%"; FLUSH PRIVILEGES;'
...
start_slave_stmt="CHANGE MASTER TO MASTER_HOST='mysql_master',MASTER_USER='db_replica_slave_user',MASTER_PASSWORD='db_replica_slave_pwd',MASTER_LOG_FILE='$CURRENT_LOG',MASTER_LOG_POS=$CURRENT_POS; START SLAVE;"
